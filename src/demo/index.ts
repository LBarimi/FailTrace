...
import { assessRun, createBundle, minimizeFailure, runTrials } from '../core/index.js';
import type { BundleResult, MinimizeEvaluation, RunStatistics, TrialResult } from '../core/index.js';
import { writeJsonAtomic } from '../core/artifacts.js';

export type DemoStage = 'repetition' | 'minimization' | 'bundle';
...
export async function runDemo(options: DemoOptions = {}): Promise<DemoResult> {
  ...
  try {
    ...
    for (const name of ['flaky-demo.js', 'advanced-demo.js', 'advanced-input.json']) {
    ...
    result.bundle = await createBundle({
      ...
      files: ['advanced-demo.js', 'package.json'], input: reduction.minimizedPath,

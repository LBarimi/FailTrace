...
Contributors can instead build a checkout with `npm ci` and `npm run build`, then run `node examples/unit-tests/prepare-unity.mjs`. For CLI calls from that checkout, use `node dist/cli/index.js` in place of `npx --yes failtrace@1.5.0`.

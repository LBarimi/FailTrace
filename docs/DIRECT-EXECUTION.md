...
```sh
npx --yes failtrace@1.4.0 minimize --input failing.json --format json --exec node --arg check.mjs --arg "{input}" --stderr-contains "TARGET_FAILURE"
```
...
```sh
npx --yes failtrace@1.4.0 run --exec node --arg tests.mjs --arg --filter --arg checkout --repeat 20
```
...
```sh
npx --yes failtrace@1.4.0 bundle <final-run-id-or-path> --file check.mjs --input <minimized-input-path> --exec node --arg check.mjs --arg "{input}"
```

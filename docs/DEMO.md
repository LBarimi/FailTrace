...
Run the published example with `npx --yes failtrace@1.5.0 demo`. It uses bundled fixtures and writes to `.failtrace/demos/<id>/`.
...
The [demo manifest](assets/demo-recording.json) and [feature-image manifest](assets/readme-scenes.json) record the CLI version, checked outcomes and asset hashes. They contain no raw logs, environment values or local machine paths. The current recordings use the verified public npm installation of 1.5.0.

...
The current 0.6.0 source candidate adds bounded Core/MCP inspection of saved run trials and output, version-pinned one-command MCP client setup, stronger installed-package checks, an expanded Verify demo, and measured adoption examples. It has not yet been published to GitHub, npm, or the MCP Registry; the 0.5.0 records above remain the latest verified public distribution evidence.
...
2. **Validate agents using the released engine.** The published 0.5.0 MCP adapter exposes all six operations through Verify and Bundle; the current 0.6.0 source candidate adds bounded run inspection as the seventh tool. Observe real Codex, Claude Code, or Cursor investigations: tool selection, bounded experiments, correct handling of failures and partial results, and evidence used after a patch. An SDK connection test establishes compatibility, not useful autonomous debugging.
3. **Shorten the path to current functionality.** The source candidate replaces global npm-root path assembly with version-pinned `npx` client setup and extends the guided demo through Verify. Validate those paths from the public package after release, then address friction observed in real-project and agent trials rather than adding more Core surface area.

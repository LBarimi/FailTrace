...
2. **Observe useful results in real projects.** The installable package, guided demo, and historical Prettier case already exist. Collect voluntary evidence of developers investigating their own failures, whether the result changed their next step, and whether they returned to the tool. Use those observations to choose improvements; private logs and telemetry are not required.
3. **Validate agents using the existing engine.** The thin MCP adapter and client guides already exist. Observe real Codex, Claude Code, or Cursor investigations: tool selection, bounded experiments, correct handling of failures and partial results, and evidence used after a patch. An SDK connection test establishes compatibility, not useful autonomous debugging.

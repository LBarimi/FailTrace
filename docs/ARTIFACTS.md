...
- Observed symbolic links, junctions and special files are not followed. Linked storage roots or ancestors are rejected. Unreadable, invalid, changed or oversized entries are reported; a scan cannot establish an atomic snapshot of an actively changing filesystem.

...
The published package includes the read-only `artifacts` command and `inventoryArtifacts` Core API. The optional storage budget described below requires version **1.4.0** or newer. [Documentation index](README.md)
...
```sh
npx --yes failtrace@1.5.0 artifacts
npx --yes failtrace@1.5.0 artifacts --cwd /path/to/project --json
```
...
- Only selected report filenames are parsed for state and known path fields. Metadata reads are bounded to 32 MiB per document and 96 MiB overall. Before JSON parsing, each report is limited to 100000 containers, scalar values and object keys combined, and 64 levels of nesting; reference traversal is also bounded. Trial logs are counted by size and never read.
...
```sh
npx --yes failtrace@1.5.0 artifacts --cwd /path/to/project --max-bytes 1073741824 --json
```
